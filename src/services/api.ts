...
export const api = {
  ...
  get: async (params?: IApiParams) => {
    ...
    if (params?.stores?.includes('dmarket')) {
      ...
      localStorage.setItem('dmarketPage', cursor ? cursor : dmarketStoragePageCursor)
      ...
      dmarketSkins = dmarketItems.map((item: IDmarketSkin) => {
        ...
        return {
          ...
          priceFormated: api.formatCurrency((Number(item.price.USD) / 100) * dolar),
    ...
    if (params?.stores?.includes('dash')) {
      ...
      dashSkins = dashItems.map((item: IDashSkin) => {
        const qualityDescription = item.market_hash_name.match(/\((.*?)\)/)?.pop()
    ...
    if (params?.stores?.includes('neshastore')) {
      ...
      try {
        ...
        neshastoreSkins = neshastoreItems.map((item: INeshastoreSkin) => {
          const lockInterval =
    ...
    if (params?.stores && params.stores.length) {
      ...
      params.stores?.includes('neshastore') && allSkins.push(...neshastoreSkins)
      params.stores?.includes('dash') && allSkins.push(...dashSkins)
    } else {
      allSkins = [...csmoneySkins, ...dmarketSkins, ...neshastoreSkins, ...dashSkins]
    }
